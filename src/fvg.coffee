...
exports.compile = compile = (src, callback) ->
	...
	eof = [ 'console.log JSON.stringify { code : jaqen, json: mask }' ]
	...
	async.waterfall [
		...
		(callback) ->
			if src.slice(-4) == '.fvg' or src.slice(-4) == '.svg'
				...
				json_0 = JSON.stringify @g_json
				json_1 = json_0.replace( /\"/g, '\\\"' )
				no_one.push "mask = #{ json_1 }", parse( src ), eof
				callback null, no_one.join ';'

		, (code, callback) ->
			exec 'coffee -e \"' + code + '\"', (error, stdout, stderr) ->
				raven '@compile[exec]', error
				@g_parsed = JSON.parse( stdout )
				@g_code = @g_parsed.code
				@g_json = @g_parsed.json
