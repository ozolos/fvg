...
(function() {
  var async, compile, don_png, don_svg, exec, execFile, fs, parse, raven, svgpng, validate;
    ...
    return async.waterfall([
      function(callback) {
        ...
        if (src.slice(-4) === '.fvg' || src.slice(-4) === '.svg') {
          ...
          json_0 = JSON.stringify(this.g_json);
        ...
        return exec('coffee -e \"' + code + '\"', function(error, stdout, stderr) {
          ...
          this.g_parsed = JSON.parse(stdout);
          this.g_code = this.g_parsed.code;
          this.g_json = this.g_parsed.json;
